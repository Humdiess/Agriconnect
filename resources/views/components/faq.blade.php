...
@foreach ($faqs as $faq)
<div x-data="{ active: false }" class="cursor-pointer group">
    <button @click="active = !active" class="flex items-center justify-between w-full p-4 text-left select-none group-hover:underline">
        <span>{{ $faq['question'] }}</span>
        <svg class="w-4 h-4 duration-200 ease-out" :class="{ 'rotate-180': active }" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
    </button>
    <div x-show="active" x-collapse x-cloak>
