            ...
            <div id="productGrid"
                ...
                {{-- @for ($i = 1; $i <= 8; $i++)
                    <div class="product-card border dark:border-zinc-600 rounded-lg overflow-hidden transition-all duration-300"
                        data-category="{{ ['alat', 'pupuk', 'bibit'][($i - 1) % 3] }}"
                        data-name="Produk Pertanian {{ $i }}" data-price="{{ rand(50000, 500000) }}">
                        <div class="product-image relative pb-[100%]">
                            <img src="{{ asset('img/placeholder-product.jpg') }}" alt="Product {{ $i }}"
                                class="absolute inset-0 w-full h-full object-cover">
                        </div>
                        <div class="product-info p-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Produk Pertanian
                                {{ $i }}</h3>
                            <p class="text-gray-600 dark:text-gray-300 mb-2">Deskripsi singkat produk pertanian
                                {{ $i }}.</p>
                            <div class="flex justify-between items-center">
                                <span class="text-xl font-bold text-accent">Rp
                                    {{ number_format(rand(50000, 500000), 0, ',', '.') }}</span>
                                <button
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterProducts() {
                ...
                Array.from(productCards).forEach(card => {
                    ...
                    const matchesCategory = selectedCategory === 'all' || productCategory ===
                        selectedCategory;
